...
# --- BRONZE LOAD LAYER ---
@asset(key=AssetKey(["bronze", "raw_customers"]), group_name="bronze", deps=[raw_csv_files], io_manager_key="bronze_io_manager")
def bronze_raw_customers(paths: PathConfig) -> pd.DataFrame:
    file_path = os.path.join(paths.raw_local_path, "raw_customers.csv")
    return pd.read_csv(file_path)

@asset(key=AssetKey(["bronze", "raw_stores"]), group_name="bronze", deps=[raw_csv_files], io_manager_key="bronze_io_manager")
def bronze_raw_stores(paths: PathConfig) -> pd.DataFrame:
    file_path = os.path.join(paths.raw_local_path, "raw_stores.csv")
    return pd.read_csv(file_path)

@asset(key=AssetKey(["bronze", "raw_products"]), group_name="bronze", deps=[raw_csv_files], io_manager_key="bronze_io_manager")
def bronze_raw_products(paths: PathConfig) -> pd.DataFrame:
    file_path = os.path.join(paths.raw_local_path, "raw_products.csv")
    return pd.read_csv(file_path)

@asset(key=AssetKey(["bronze", "raw_supplies"]), group_name="bronze", deps=[raw_csv_files], io_manager_key="bronze_io_manager")
def bronze_raw_supplies(paths: PathConfig) -> pd.DataFrame:
    file_path = os.path.join(paths.raw_local_path, "raw_supplies.csv")
    return pd.read_csv(file_path)

@asset(key=AssetKey(["bronze", "raw_items"]), group_name="bronze", deps=[raw_csv_files], io_manager_key="bronze_io_manager")
def bronze_raw_items(paths: PathConfig) -> pd.DataFrame:
    file_path = os.path.join(paths.raw_local_path, "raw_items.csv")
    return pd.read_csv(file_path)

@asset(key=AssetKey(["bronze", "raw_orders"]), group_name="bronze", deps=[raw_csv_files], io_manager_key="bronze_io_manager")
def bronze_raw_orders(paths: PathConfig) -> pd.DataFrame:
    file_path = os.path.join(paths.raw_local_path, "raw_orders.csv")
    return pd.read_csv(file_path)

@asset(key=AssetKey(["bronze", "support_tickets"]), group_name="bronze", deps=[raw_jsonl_files], io_manager_key="bronze_io_manager")
...
bronze_assets = [raw_csv_files, raw_jsonl_files, bronze_raw_customers, bronze_raw_stores, bronze_raw_products, bronze_raw_supplies, bronze_raw_items, bronze_raw_orders, bronze_support_tickets]
